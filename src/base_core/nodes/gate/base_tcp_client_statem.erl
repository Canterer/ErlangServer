...
?init([OnReceiveData,OnClientClose]) ->
	...
	{A,B,C} = os:timestamp(),
	base_logger_util:info_msg("~p:~p os:timestamp=~p~n",[?MODULE,?FUNCTION_NAME,os:timestamp()]),
...
% 	base_logger_util:info_msg("~p:~p(EventType=~p,EventContent=~p,StateData=~p)~n",[?MODULE,?FUNCTION_NAME,EventType,EventContent,StateData]),
			...
			FPacketProcRe = 
				case gen_tcp:recv(CliSocket,4,180000) of			%%timeout 3 min
					{ok,?CROSS_DOMAIN_FLAG_HEADER}->
						?ZS_LOG(),
						base_logger_util:info_msg("client port policy-file-request failed,game proc ~n"),
					...
					{ok, <<"tgw_">>} ->
						...
						send_data(self(), login_pb:encode_proto_msg(tgw_gateway_s2c,#tgw_gateway_s2c{})),
						true; 
					{ok,RecvBin}->
						?ZS_LOG(),
						<< PacketLenth:?PACKAGE_HEADER_BIT_LENGTH/big,LeftHeaderBin/binary >> = RecvBin,
						base_logger_util:info_msg("PacketLenth ~p RecvBin ~p ~n",[PacketLenth,RecvBin]),
						...
						case gen_tcp:recv(CliSocket,PacketLenth - 2,10000) of
							...
							Errno2->
								...
								base_logger_util:info_msg("socket_ready recv ERROR ~p ip ~p !!!! ~n",[Errno2,PeerIPAddressTmp]),
								false
						end;
					{error,closed}->
						?ZS_LOG(),
						stop;
					Errno->
						base_logger_util:info_msg("socket_ready recv ERROR ~p !!!! ~n",[Errno]),
						false
						
				end,
				%base_logger_util:info_msg("yanzengyan, FPacketProcRe: ~p ~n",[FPacketProcRe]),
				case FPacketProcRe of
					...
					false->
						base_logger_util:info_msg("first packet proc failed,kick client ~n"),
						put(clientsock, CliSocket),
...
?handle_event(cast,{env_prepared,Info},preparing_into_map,StateData)->
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
?handle_event(cast, stop, _StateName, _StateData)->
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
?handle_event({call, From}, {role_process_started,MapNode,RoleProc}, StateName, StateData) ->
...
% handle_info({mapid_change,MapNode,MapId,RoleProc}, StateName, StateData) ->
	...
	keep_state_and_data;
?handle_event(info, {alive_check}, StateName, StateData) ->
...
% handle_info({send_to_client, Data}, StateName, StateData) ->
	...
	try erlang:binary_to_term(Data) of
		Val-> base_logger_util:info_msg("send_to_role ~p~n",[erlang:binary_to_term(Data)])
	catch
		_:_->
...
%% 			base_logger_util:info_msg("send_to_role@@ [~p]~n",[ID])
	...
	% {next_state, StateName, StateData};
	keep_state_and_data;
?handle_event(info, {send_to_client_filter, Cur_Binary,Flt_Binary}, StateName, StateData) ->
% handle_info({send_to_client_filter, Cur_Binary,Flt_Binary}, StateName, StateData) ->
	...
	case IsFilter of
		true->
			%%@@ base_logger_util:info_msg("send_to_role ~p~n",[erlang:binary_to_term(Flt_Binary)]),
			erlang:port_command(get(clientsock), Flt_Binary, [force]);
		_->
			%%@@ base_logger_util:info_msg("send_to_role ~p~n",[erlang:binary_to_term(Cur_Binary)]),
			erlang:port_command(get(clientsock), Cur_Binary, [force])
	end,
	
	% {next_state, StateName, StateData};
	keep_state_and_data;
?handle_event(info, {do_recv}, StateName, StateData) ->
...
% handle_info({tcp, Socket, BinData}, StateName, StateData) ->
	...
	try	
		...
		Type:Reason ->
				base_logger_util:info_msg("tcp recv exception: ~p BinData~p ~n",[{Type,Reason},BinData]),
...
send_left_msg()->
	case get(message_queue) of
		...
		Any ->
			...
			base_logger_util:info_msg("send_to_role ~p~n",[Any]),
...
%% 启动聊天进程
start_chat_role()->
   base_logger_util:info_msg("start_chat_role~n"),
   case base_line_manager_server:get_chat_name() of
		[]->
			base_logger_util:info_msg("start_chat_role get chatnode name err!!!!!!~n"),
			{node(),self()};
			...
			S = fun(DataBin)->
				try
					apply(M,F,A++[self(), DataBin, Pid])
						...
						Type:Reason ->
							base_logger_util:info_msg("RoleId: ~p tcp do_recv error: ~p ~p~n",[get(roleid),{Type,Reason},erlang:get_stacktrace()])
				end,
