...
start_link(Port, AcceptorCount, OnStartup, OnShutdown) ->
	?base_gen_server:start_link({local, ?SERVER}, ?MODULE, {Port, AcceptorCount, OnStartup, OnShutdown}, []).
...
?init({Port,AcceptorCount,{M,F,A} = OnStartup, OnShutdown}) ->
	...
	case gen_tcp:listen(Port, Opts) of
		{ok, Listen_socket} ->
			...
			{ok, #state{listen_socket = Listen_socket,on_startup = OnStartup, on_shutdown = OnShutdown,acceptors=AccProcs}};
		{error, Reason} ->
			{stop, Reason}
	end.
