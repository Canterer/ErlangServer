...
start_link(RoleStartInfo,RoleId)->
	...
	base_fsm_util:start_link({local, Role_proc_name}, ?MODULE, [RoleStartInfo,RoleId, Role_proc_name], []).
...
role_move_request(RolePid,MoveInfo) ->
	base_fsm_util:send_state_event(RolePid, {role_move_request,MoveInfo}).

stop_move_c2s(RolePid, MoveInfo)->
	base_fsm_util:send_state_event(RolePid, {stop_move_c2s,MoveInfo}).
%% 事件: 客户端地图加载完成
...
uninit_role(Role_node, Role_proc) ->
	base_fsm_util:send_state_event(Role_node, Role_proc, {uninit_role}).

%% 事件: 发起点攻击
start_attack(RolePid, SkillID, TargetID) ->
	base_fsm_util:send_state_event(RolePid, {start_attack, {SkillID, TargetID}}).
...
role_respawn(RolePid,Type)->
	base_fsm_util:send_state_event(RolePid,{role_respawn,Type}).
...
set_leader_to_you(Node,RolePid,GroupInfo)->
	try
		base_fsm_util:sync_send_all_state_event({RolePid,Node}, {set_leader_to_you,GroupInfo})
	catch
		_E:_R->error
	end.
